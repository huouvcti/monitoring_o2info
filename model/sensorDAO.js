...
sensor_log.list = (parameters) => {
    return new Promise((resolve, reject) =>{
        db.query(`SELECT format(Tc, 2) as Tc, format(DO, 2) as DO, format(DOper, 2) as DOper, format(pH, 2) as pH, format(Sa, 2) as Sa, format(ORP, 2) as ORP, format(TUR, 2) as TUR, format(CHL, 2) as CHL, DATE_FORMAT(date, '%Y-%m-%d %T') as date FROM sensor WHERE (user_key=?) AND (date > ? AND date < ?) ORDER BY date DESC LIMIT ?, ?`, [ parameters.user_key, parameters.date_start, parameters.date_end, parameters.offset, parameters.limit], (err, db_data) => {
            if(err) {
                reject(err);
            } else {
                resolve(db_data);
